...
Two columns have been created: `taxa` (taxon names) and `counts` (taxa counts).
...
**Note:** this function requires data in a long format (see `reshape_data()`).

It takes a `data.frame` and a vector of species names (argument `species`).

Let's subset plankton net data (in long format)to only keep observations for *G. glutinata* and *C. nitida*.

```{r 'filter-by-species'}
# Filter by species ----
glutinata_nitida <- filter_by_species(net_data_long, 
