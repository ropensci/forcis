...
test_that("Test read_sediment_trap_data() for error", {
  ...
  expect_error(
    suppressWarnings(read_sediment_trap_data(version = "07")),
    paste0(
  ...
  expect_error(
    suppressWarnings(read_sediment_trap_data(version = "07")),
    paste0(
      "The Sediment trap dataset does not exist. Please use ",
...
test_that("Test read_sediment_trap_data() for success", {
  ...
  write.csv2(
    df,
    file = file.path("forcis-db", "version-07", "FORCIS_trap_test.csv"),
    row.names = FALSE
  )

  x <- suppressWarnings(
    read_sediment_trap_data(version = "07", check_for_update = FALSE)
  )
  ...
  expect_message(
    {
      x <- suppressWarnings(
        read_sediment_trap_data(version = "07")
      )
    },
    paste0(
