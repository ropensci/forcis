...
test_that("Test read_plankton_nets_data() for error", {
  ...
  expect_error(
    suppressWarnings(read_plankton_nets_data(version = "07")),
    paste0(
...
test_that("Test read_plankton_nets_data() for success", {
  ...
  write.csv2(
    df,
    file = file.path("forcis-db", "version-07", "FORCIS_net_test.csv"),
    row.names = FALSE
  )

  x <- suppressWarnings(
    read_plankton_nets_data(version = "07", check_for_update = FALSE)
  )
