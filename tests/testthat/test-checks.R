...
test_that("Test check_if_character() for error", {
  ...
  expect_error(check_if_character(),
               "Argument '' is required",
               fixed = TRUE)
  
  expect_error(check_if_character(12),
               "Argument '12' must be a character",
               fixed = TRUE)
  
  expect_error(check_if_character(c("a", "b")),
...
test_that("Test check_if_path_exists() for error", {
  ...
  expect_error(check_if_path_exists("path"),
...
test_that("Test check_if_df() for error", {
  ...
  expect_error(check_if_df(),
               "Argument 'data' is required",
               fixed = TRUE)
  
  expect_error(check_if_df(NA),
               "Argument 'data' must be a data.frame",
               fixed = TRUE)
  
  expect_error(check_if_df(1:5),
               "Argument 'data' must be a data.frame",
               fixed = TRUE)
  
  expect_error(check_if_df(letters),
               "Argument 'data' must be a data.frame",
               fixed = TRUE)
  
  expect_error(check_if_df(TRUE),
               "Argument 'data' must be a data.frame",
               fixed = TRUE)
  
  expect_error(check_if_df(NULL),
               "Argument 'data' must be a data.frame",
               fixed = TRUE)
  
  expect_error(check_if_df(list()),
               "Argument 'data' must be a data.frame",
               fixed = TRUE)
  
  expect_error(check_if_df(matrix(1:4, ncol = 2)),
               "Argument 'data' must be a data.frame",
               fixed = TRUE)
  
  expect_error(check_if_df(data.frame()),
...
test_that("Test check_field_in_data() for error", {
  ...
  expect_error(check_field_in_data(df, "z"),
...
test_that("Test check_required_columns() for error", {
  ...
  expect_error(check_required_columns(df_w_missing_req_col),
...
test_that("Test check_if_valid_taxonomy() for error", {
  ...
  expect_error(check_if_valid_taxonomy("AA"),
...
test_that("Test check_unique_taxonomy() for error", {
  ...
  expect_error(check_unique_taxonomy(df),
               paste0("Multiple taxonomies are not allowed. Please use the ", 
