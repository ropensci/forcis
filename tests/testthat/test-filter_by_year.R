...
df_net <- data.frame("data_type" = rep("Net", 6),
                     "profile_date_time" = c("01/01/1999", "01/01/2020",
                                             "01/01/2022", "01/01/2023",
                                             "01/01/2024", NA))

df_trap <- data.frame("data_type" = rep("Sediment trap", 6),
                      "sample_date_time_start" = c("01/01/1999", "01/01/2020",
                                                   "01/01/2022", "01/01/2023",
                                                   "01/01/2024", NA))

df_trap_na <- data.frame("data_type"              = rep("Sediment trap", 6),
                         "sample_date_time_start" = rep(NA, 6))


df_net_na <- data.frame("data_type"         = rep("Net", 6),
...
test_that("Test filter_by_year() for error", {
  ...
  expect_error(filter_by_year(df_net),
               "Argument 'years' is required",
               fixed = TRUE)
  
  expect_error(filter_by_year(df_net, year = "2000"),
  ...
  expect_error(filter_by_year(df_net, year = 2000:2019),
               "The years provided are out of FORCIS temporal range",
               fixed = TRUE)
  
  expect_error(filter_by_year(df_trap, year = 2000),
               "The years provided are out of FORCIS temporal range",
               fixed = TRUE)
  
  expect_error(filter_by_year(df_trap, year = 2000:2019),
               "The years provided are out of FORCIS temporal range",
               fixed = TRUE)
  
  expect_error(filter_by_year(df_trap_na, year = 2000),
               "The column 'sample_date_time_start' contain only NA",
               fixed = TRUE)
  
  expect_error(filter_by_year(df_net_na, year = 2000),
