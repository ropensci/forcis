...
test_that("Test filter_by_month() for error", {
  ...
  expect_error(filter_by_month(df_net),
               "Argument 'months' is required",
               fixed = TRUE)
  
  expect_error(filter_by_month(df_net, months = "02"),
               "Argument 'months' must be a numeric of length >= 1",
               fixed = TRUE)
  
  expect_error(filter_by_month(df_net, months = 1),
               "The months provided are out of FORCIS temporal range",
               fixed = TRUE)
  
  expect_error(filter_by_month(df_net, months = 10:11),
               "The months provided are out of FORCIS temporal range",
               fixed = TRUE)
  
  expect_error(filter_by_month(df_trap, months = 1),
               "The months provided are out of FORCIS temporal range",
               fixed = TRUE)
  
  expect_error(filter_by_month(df_trap, months = 10:11),
               "The months provided are out of FORCIS temporal range",
               fixed = TRUE)
  
  expect_error(filter_by_month(df_trap_na, months = 1),
               "The column 'sample_date_time_start' contain only NA",
               fixed = TRUE)
  
  expect_error(filter_by_month(df_net_na, months = 1),
