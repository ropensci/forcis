...
compute_concentrations <- function(data, aggregate = TRUE) {
  ...
  cols_to_remove <- c("subsample_count_type", "sampling_device_type",
  ...
  cols_to_remove <- c(
    "counts", "abs_counts", "subsample_count_type",
    "subsample_all_shells_present_were_counted", "total_of_forams_counted_ind",
  ...
  if (aggregate) {
    ...
    cols_to_remove <- c(
      "counts_n_conc", "abs_sub_tot", "subsample_id", 
      "subsample_size_fraction_min", "subsample_size_fraction_max", "taxa"
    )
